...
- [x] 设计并迁移`users`、`sms_challenges`、`sessions`和服务端`learning_states`。
- [ ] 接入腾讯云短信SDK 3.0适配器，完成频控、验证码哈希、有效期和尝试次数。（**defer**：dev mock；见 completion 摘要）
- [x] 实现高熵opaque session、数据库token哈希和Android Keystore保存。
- [x] 实现单主设备事务切换和旧会话撤销。（跨设备策略见 ADR 0009 §1a）
- [x] 实现`sync_outbox`批量上传、事件ID幂等、客户端版本条件更新和确认后删除。
- [x] 实现服务器快照下载以及换机恢复；明确展示只能恢复到最后成功同步状态。
- [x] 验证弱网、重复、乱序、超时、App被杀和服务器恢复后继续上传。（集成测试 + 维护手册；弱网无独立 E2E）
