...
```text
阶段 0–4  ✅（本地学习闭环；sync_outbox 仅本地写入）
阶段 5    ✅ 已确认 kickoff（待子计划与实施）
阶段 6–8  ⏸
```
...
| 首次安装登录引导（Q1:B） | **真做**：首次打开可展示轻量引导页，含「登录」与「稍后」；选稍后则与阶段 4 相同，不反复强制 |
...
| 题 | 你的选择 | 落地含义 |
| --- | --- | --- |
| Q1 | **B** | 首次安装可展示登录引导，允许「稍后」跳过；跳过后不反复强制，本地学习照常 |
